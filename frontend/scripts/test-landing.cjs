...
(async () => {
  ...
  await page.getByText('$72.00',{exact:true}).waitFor();
  await page.getByRole('button', {name:'Next testimonial'}).click();
  await page.getByText('Story 2 / 21', {exact:true}).waitFor();
  await page.getByRole('button', {name:'Previous testimonial'}).click();
  await page.getByRole('button', {name:'Previous testimonial'}).click();
  await page.getByText('Story 21 / 21', {exact:true}).waitFor();
  await page.getByRole('button', {name:'Next testimonial'}).click();
  await page.getByText('Story 1 / 21', {exact:true}).waitFor();
