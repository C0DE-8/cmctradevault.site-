...
async function migrate() {
  await migrateUsers();
  await createTable("binary_trades", `CREATE TABLE IF NOT EXISTS binary_trades (
    ...
    payout_percent DECIMAL(5,2) NOT NULL DEFAULT 80,
    pnl DECIMAL(18,2) NULL,
    status VARCHAR(20) NOT NULL DEFAULT 'open',
    opened_at_ms BIGINT NOT NULL,
    expires_at_ms BIGINT NOT NULL,
    settled_at_ms BIGINT NULL,
    KEY binary_user_status (user_id,status),
    KEY binary_expiry (status,expires_at_ms)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`);
  await createTable("admin_audit_logs", `CREATE TABLE IF NOT EXISTS admin_audit_logs (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    KEY audit_admin_created (admin_id, created_at)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`);
  ...
  await addColumn("user_kyc", "approved_by", "INT UNSIGNED NULL");
  await addColumn("user_kyc", "approved_at", "DATETIME NULL");
  await addColumn("user_kyc", "declined_by", "INT UNSIGNED NULL");
  await addColumn("user_kyc", "declined_at", "DATETIME NULL");

  await run("ALTER TABLE trades MODIFY COLUMN entry_price DECIMAL(28,8) NULL");
  await run("ALTER TABLE trades MODIFY COLUMN duration VARCHAR(20) NOT NULL DEFAULT '1m'");
  ...
  await addColumn("users", "copied_trader_id", "INT UNSIGNED NULL");
  ...
  await createTable("balance_adjustments", `CREATE TABLE IF NOT EXISTS balance_adjustments (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY, user_id INT UNSIGNED NOT NULL,
    admin_id INT UNSIGNED NOT NULL, balance_key VARCHAR(32) NOT NULL,
    amount DECIMAL(28,8) NOT NULL, before_balance DECIMAL(28,8) NOT NULL,
    after_balance DECIMAL(28,8) NOT NULL, reason VARCHAR(250) NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, KEY(user_id, id)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`);
  const plans = [
