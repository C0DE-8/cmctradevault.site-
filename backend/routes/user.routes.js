...
router.post("/investments", auth, async (req, res) => {
  ...
  try {
    ...
    if (req.body?.risk_acknowledged !== "yes" || req.body?.terms_acknowledged !== "yes") {
      return res.status(400).json({ message: "Please acknowledge investment risk and accept the Terms & Investment Policy before investing." });
    }
    ...
    const [uRows] = await conn.query(
      `
      SELECT id, main_balance, investment_balance, withdraw_hold
      FROM users
      WHERE id = ?
      FOR UPDATE
      `,
    ...
    const user = uRows[0];
    const mainBal = Math.max(0, (Number(user.main_balance) || 0) - (Number(user.withdraw_hold) || 0));
