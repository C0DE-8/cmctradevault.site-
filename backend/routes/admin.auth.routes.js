...
router.post("/login", async (req, res) => {
  try {
    ...
    req.auditActor = {id:admin.id,email:admin.email};
...
router.patch("/profile", auth, adminOnly, async (req, res) => {
  ...
  if (name.length < 2 || name.length > 150)
    return res.status(400).json({ message: "Name must be 2–150 characters." });
  ...
  try {
    ...
    if (newPassword && !(await verifyPassword(currentPassword, admins[0].password_hash)))
    ...
    const token = signToken({ id: req.user.id, role: "admin", email });
    res.json({
      message: newPassword ? "Profile and password updated." : "Profile updated.",
      token,
      admin: { id: req.user.id, name, email },
    });
  } catch (err) {
    if (err.code === "ER_DUP_ENTRY")
      return res.status(409).json({ message: "That email is already in use." });
    console.error("[admin.profile] failed:", err);
...
router.put("/users/:id", auth, adminOnly, async (req, res) => {
  try {
    ...
    if (["main_balance","profit_balance","investment_balance","crypto_balances"].some(key => Object.hasOwn(req.body,key)))
      return res.status(400).json({message:"Use the balance-adjustments endpoint to change balances with an audit reason."});
